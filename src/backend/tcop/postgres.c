 ...
 * at soonest convenient time 系统关闭？
 */
void
die(SIGNAL_ARGS)
{
...
ProcessInterrupts(void)
{
	...
	if (IdleInTransactionSessionTimeoutPending)
	{
		...
		if (IdleInTransactionSessionTimeout > 0)
		{
			...
			ereport(FATAL,
					(errcode(ERRCODE_IDLE_IN_TRANSACTION_SESSION_TIMEOUT),		// 超时
...
PostgresMain(const char *dbname, const char *username)
{
	 ...
	 * SIGPIPE.) 设置各种信号的处理函数，例如SIGHUP用于重新加载配置文件，SIGINT用于取消当前查询，SIGTERM用于终止进程等。
	 */
	if (am_walsender)
		WalSndSignals();
	else
	 ...
	 * 如果需要在此附近添加代码，最好将其放入InitPostgres()函数
	...
	for (;;)
	{
		...
		switch (firstchar)
		{
			...
			case PqMsg_Terminate:	// 客户端希望主动关闭连接 // 登出？
				 ...
				 * scenarios.
				 * 如果需要在后端进程关闭时执行某些操作，应该使用 on_proc_exit 或 on_shmem_exit 回调机制，而不是直接在这里添加代码。
				 * 原因是，如果直接在这里添加代码，这些代码可能不会在其他后端关闭场景中被执行（例如，进程因信号或其他原因终止）。
				 */
				proc_exit(0); // 系统关闭？
