...
/*
 * ProcessUtility
 *		general utility function invoker 通用工具函数调用器
 *
 *	pstmt: PlannedStmt wrapper for the utility statement 封装了工具语句的 PlannedStmt 对象
 *	queryString: original source text of command 命令的原始文本源码
 *	readOnlyTree: if true, pstmt's node tree must not be modified 若为 true，则禁止修改 pstmt 的节点树
 *	context: identifies source of statement (toplevel client command,         标识语句来源（如：顶层客户端命令、
 *		non-toplevel client command, subcommand of a larger utility command)  非顶层命令、其他工具命令的子命令）
 *	params: parameters to use during execution 执行时使用的参数
 *	queryEnv: environment for parse through execution (e.g., ephemeral named 解析到执行期间的环境
 *		tables like trigger transition tables).  May be NULL.  （例如触发器转换表等临时命名表），可为 NULL
 *	dest: where to send results 结果输出位置
 *	qc: where to store command completion status data.  May be NULL,
 *		but if not, then caller must have initialized it. 存储命令完成状态的数据结构
 *
 ...
 * perhaps "(query not available)". 调用者必须提供 queryString
 *
			   ...
			   QueryCompletion *qc)
{
	 ...
	 * call standard_ProcessUtility(). 允许第三方插件在ProcessUtility函数被调用时劫持控制权
	 */
	if (ProcessUtility_hook)	// 执行插件注册的钩子函数
 ...
 * standard_ProcessUtility：处理 不支持事件触发器 的简单实用命令（比如 START TRANSACTION）。
 * ProcessUtilitySlow：处理 支持事件触发器 的复杂命令（比如 CREATE TABLE），因为需要额外的事件触发逻辑。
 */
void
