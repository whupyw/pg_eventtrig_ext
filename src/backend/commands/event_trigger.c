...
CreateEventTrigger(CreateEventTrigStmt *stmt)
{
	...
	/* Validate event name. 检查事件名称是否合法*/
	// 新增支持的事件触发器类型，需在此处添加对应的 strcmp 分支
	...
	/* Validate filter conditions.  CREATE TRIGGER ... WHEN (tag = 'xxx') 确保过滤条件中只允许使用预定义的tag参数*/
	...
	funcoid = LookupFuncName(stmt->funcname, 0, NULL, false);	//通过funcname在系统目录（如 pg_proc）中查找对应的函数OID
...
/*
 * Insert the new pg_event_trigger row and record dependencies. 将触发器信息写入pg_event_trigger系统表
 */
static Oid
						   ...
						   Oid funcoid, List *taglist)
{
	 ...
	 * 登录事件触发器在 pg_database 系统表中有一个额外标志位，
 	 * 用于在热点代码路径（hot codepaths）中快速判断是否存在登录触发器，
	 * 避免重复查询系统目录表。若标志位未设置，则在此处设置。
	 * 
	 * 登录操作（如用户连接数据库）是高频操作。若每次登录都要查询 pg_event_trigger 系统表来检查是否有登录事件触发器，
	 * 会产生显著的开销。通过 pg_database 中的标志位，只需一次位检查即可跳过复杂查询。
	 * 标志位仅在创建登录触发器时设置一次，后续登录操作直接依赖该标志位。
	 * 即使触发器被删除，标志位也不会立刻清除，而是通过事件触发器本身的失效机制（如 pg_event_trigger 条目删除）隐式处理。
	 */
...
EventTriggerDDLCommandStart(Node *parsetree)
{
	 ...
	 */
	if (!IsUnderPostmaster || !event_triggers)	// 独立模式下禁用事件触发器
		return;
 ...
 * 当删除一个事件触发器时，pg_database表中的dathasloginevt标志位不会被修改，以避免在有多个事件触发器的情况下使代码路径变得复杂。
 * 相反，这个函数会在没有定义触发器时清除该标志位。
 */
void
EventTriggerOnLogin(void)
{
	 ...
	 * 执行条件：多进程、启用了事件触发器、数据库连接有效、有登录事件触发器
	 */
	if (!IsUnderPostmaster || !event_triggers ||
	 ...
	 * just trying to acquire the lock conditionally. 如果没有触发器存在，尝试清除dathasloginevt标志
